...
# ⚠️ CRITICAL — import main FIRST so pickle can find FeaturePipeline etc.
import main
from main import FeaturePipeline, StyleMatrixExtractor, GujaratiTokenizer
...
def load_all_ml_models():
    """Load every .pkl from the repo folder. `main.py` provides the classes."""
    models = {}
    status = []
...
with st.sidebar:
    ...
    st.info(
        ...
        "Classes for unpickling live in `main.py`."
    )
...
if analyze_btn:
    ...
    if not ALL_ML_MODELS:
        st.error(
            "⚠️ **0 ML models loaded.** Check sidebar **❌ Failed** expander."
        )
    else:
